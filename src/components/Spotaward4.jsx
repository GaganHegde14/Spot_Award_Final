...
import SpotAwardForm from "./SpotAwardForm4.jsx";
import QuotaDetails from "./QuotaDetails4.jsx";
import QuotaTableRow from "./QuotaTableRow4.jsx";
...
function SpotAwardRow({ row, isSelected, onSelect, onAwardIssuedClick }) {
...
const SpotAwardRequest = ({ data = defaultData, onSelectionChange }) => {
  ...
  const [isBigPopupOpen, setIsBigPopupOpen] = useState(false);
  const [bigPopupTrigger, setBigPopupTrigger] = useState(null);
  ...
  function handleAwardIssuedClick(rowData, event) {
    if (rowData.awardIssued > 0) {
      setBigPopupTrigger(event.target);
      setIsBigPopupOpen(true);
    }
  }

  function handleCloseBigPopup() {
  ...
  return (
    <div style={ui.container}>
            ...
            <tbody>
              {rows.map((row) => (
                <SpotAwardRow
                  ...
                  onAwardIssuedClick={handleAwardIssuedClick}
                />
              ))}
            </tbody>
          </table>
      ...
      <BigPopup
        isOpen={isBigPopupOpen}
        onClose={handleCloseBigPopup}
        triggerElement={bigPopupTrigger}
      />
    </div>
  );
};
